        ...
        <v-flex offset-md4 md4>
          ...
          >
            <v-card-title class="justify-center" primary-title>
              <div v-if="title.title" class="headline">{{ title.title }}</div>
            </v-card-title>
          </v-card>

          <div
            v-if="!loading"
            v-for="card in randomOptions"
            :key="card.card_id"
          >
            <v-btn
  ...
  methods: {
    randomize: async function() {
      ...
      this.randomOptions = this.cards
        ...
        .slice(0, 4);
      console.log(this.randomOptions.length);
      console.log(Math.floor(Math.random() * (this.randomOptions.length - 1)));
